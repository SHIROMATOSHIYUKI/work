...
// 受けるがわ。レイ対メッシュ
bool EnemyBase::CheckCollisionDamegeRay(const RayInfo& info, DamegeResult& result)
{
	...
	for (UINT i = 0; i < m_modelWork.GetDataNodes().size(); ++i)
	{
		...
		if (rayResult.m_hit && rayResult.m_distance < minDist)		// レイがtrueか&&最初に当たったメッシュ
		{
